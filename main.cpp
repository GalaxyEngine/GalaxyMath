...
#pragma region Matrix 4 Tests
    ...
    NAMESPACE(Matrix_4)
    {
        ...
        TEST(TransformationMethods)
        {
            ...
            glm::mat4 glmRotationMatrix =
                glm::eulerAngleXYZ(DegToRad * euler.x, DegToRad * euler.y, DegToRad * euler.z);
...
#pragma region Hash Tests
    NAMESPACE(Hash)
    {
        TEST(Vec2_Hash)
        {
            std::hash<Vec2f> hasher;

            // Same values produce same hash
            REQUIRE(hasher(Vec2f(1.f, 2.f)) == hasher(Vec2f(1.f, 2.f)));

            // Different values produce different hashes
            REQUIRE(hasher(Vec2f(1.f, 2.f)) != hasher(Vec2f(2.f, 1.f)));
            REQUIRE(hasher(Vec2f(1.f, 2.f)) != hasher(Vec2f(1.f, 3.f)));
            REQUIRE(hasher(Vec2f(0.f, 0.f)) != hasher(Vec2f(1.f, 0.f)));

            // Works with different underlying types
            std::hash<Vec2i> hasherI;
            REQUIRE(hasherI(Vec2i(3, 6)) == hasherI(Vec2i(3, 6)));
            REQUIRE(hasherI(Vec2i(3, 6)) != hasherI(Vec2i(6, 3)));

            // Usable as unordered_map key
            std::unordered_map<Vec2f, int> map;
            map[Vec2f(1.f, 2.f)] = 42;
            map[Vec2f(3.f, 4.f)] = 99;
            REQUIRE(map[Vec2f(1.f, 2.f)] == 42);
            REQUIRE(map[Vec2f(3.f, 4.f)] == 99);
            COMPARE((int)map.size(), 2);
        }
        ...
        TEST(Vec3_Hash)
        {
            ...
            // Same values produce same hash
            REQUIRE(hasher(Vec3f(1.f, 2.f, 3.f)) == hasher(Vec3f(1.f, 2.f, 3.f)));

            // Different values produce different hashes
            REQUIRE(hasher(Vec3f(1.f, 2.f, 3.f)) != hasher(Vec3f(3.f, 2.f, 1.f)));
            REQUIRE(hasher(Vec3f(1.f, 2.f, 3.f)) != hasher(Vec3f(1.f, 2.f, 4.f)));
            REQUIRE(hasher(Vec3f(0.f, 0.f, 0.f)) != hasher(Vec3f(1.f, 0.f, 0.f)));

            // Works with different underlying types
            std::hash<Vec3i> hasherI;
            REQUIRE(hasherI(Vec3i(1, 2, 3)) == hasherI(Vec3i(1, 2, 3)));
            REQUIRE(hasherI(Vec3i(1, 2, 3)) != hasherI(Vec3i(3, 2, 1)));

            // Usable as unordered_map key
            std::unordered_map<Vec3f, std::string> map;
            map[Vec3f(1.f, 0.f, 0.f)] = "right";
            map[Vec3f(0.f, 1.f, 0.f)] = "up";
            map[Vec3f(0.f, 0.f, -1.f)] = "forward";
            REQUIRE(map[Vec3f(1.f, 0.f, 0.f)] == std::string("right"));
            REQUIRE(map[Vec3f(0.f, 1.f, 0.f)] == std::string("up"));
            REQUIRE(map[Vec3f(0.f, 0.f, -1.f)] == std::string("forward"));
            COMPARE((int)map.size(), 3);
        }

        TEST(Vec4_Hash)
        {
            std::hash<Vec4f> hasher;

            // Same values produce same hash
            REQUIRE(hasher(Vec4f(1.f, 2.f, 3.f, 4.f)) == hasher(Vec4f(1.f, 2.f, 3.f, 4.f)));

            // Different values produce different hashes
            REQUIRE(hasher(Vec4f(1.f, 2.f, 3.f, 4.f)) != hasher(Vec4f(4.f, 3.f, 2.f, 1.f)));
            REQUIRE(hasher(Vec4f(1.f, 2.f, 3.f, 4.f)) != hasher(Vec4f(1.f, 2.f, 3.f, 5.f)));
            REQUIRE(hasher(Vec4f(0.f, 0.f, 0.f, 0.f)) != hasher(Vec4f(0.f, 0.f, 0.f, 1.f)));

            // Works with different underlying types
            std::hash<Vec4i> hasherI;
            REQUIRE(hasherI(Vec4i(1, 2, 3, 4)) == hasherI(Vec4i(1, 2, 3, 4)));
            REQUIRE(hasherI(Vec4i(1, 2, 3, 4)) != hasherI(Vec4i(4, 3, 2, 1)));

            // Usable as unordered_map key
            std::unordered_map<Vec4f, int> map;
            map[Vec4f(1.f, 2.f, 3.f, 4.f)] = 1;
            map[Vec4f(4.f, 3.f, 2.f, 1.f)] = 2;
            REQUIRE(map[Vec4f(1.f, 2.f, 3.f, 4.f)] == 1);
            REQUIRE(map[Vec4f(4.f, 3.f, 2.f, 1.f)] == 2);
            COMPARE((int)map.size(), 2);
        }

        TEST(Quat_Hash)
        {
            std::hash<Quat> hasher;

            // Same values produce same hash
            REQUIRE(hasher(Quat(1.f, 2.f, 3.f, 4.f)) == hasher(Quat(1.f, 2.f, 3.f, 4.f)));
            REQUIRE(hasher(Quat::Identity()) == hasher(Quat(0.f, 0.f, 0.f, 1.f)));

            // Different values produce different hashes
            REQUIRE(hasher(Quat(1.f, 2.f, 3.f, 4.f)) != hasher(Quat(4.f, 3.f, 2.f, 1.f)));
            REQUIRE(hasher(Quat(1.f, 0.f, 0.f, 1.f)) != hasher(Quat(0.f, 1.f, 0.f, 1.f)));

            // Usable as unordered_map key
            std::unordered_map<Quat, std::string> map;
            Quat q1 = Quat::AngleAxis(90.f, Vec3f::Up());
            Quat q2 = Quat::AngleAxis(45.f, Vec3f::Right());
            map[q1] = "rotate_y_90";
            map[q2] = "rotate_x_45";
            REQUIRE(map[q1] == std::string("rotate_y_90"));
            REQUIRE(map[q2] == std::string("rotate_x_45"));
            COMPARE((int)map.size(), 2);
        }

        TEST(Mat3_Hash)
        {
            std::hash<Mat3> hasher;

            // Same values produce same hash
            REQUIRE(hasher(Mat3::Identity()) == hasher(Mat3::Identity()));
            REQUIRE(hasher(Mat3(1.f)) == hasher(Mat3(1.f)));

            // Different values produce different hashes
            Vec3f euler(32.5f, -63.21f, 17.93f);
            Mat3 rotation = Mat3::CreateRotationMatrix(euler);
            REQUIRE(hasher(Mat3::Identity()) != hasher(rotation));
            REQUIRE(hasher(Mat3(1.f)) != hasher(Mat3(2.f)));

            // Usable as unordered_map key
            std::unordered_map<Mat3, int> map;
            Vec3f scale(1.f, 2.f, 3.f);
            Mat3 scaleMatrix = Mat3::CreateScaleMatrix(scale);
            map[Mat3::Identity()] = 0;
            map[rotation] = 1;
            map[scaleMatrix] = 2;
            REQUIRE(map[Mat3::Identity()] == 0);
            REQUIRE(map[rotation] == 1);
            REQUIRE(map[scaleMatrix] == 2);
            COMPARE((int)map.size(), 3);
        }

        TEST(Mat4_Hash)
        {
            std::hash<Mat4> hasher;

            // Same values produce same hash
            REQUIRE(hasher(Mat4::Identity()) == hasher(Mat4::Identity()));
            REQUIRE(hasher(Mat4(1.f)) == hasher(Mat4(1.f)));

            // Different values produce different hashes
            Vec3f translation(1.f, 2.f, 3.f);
            Vec3f euler(32.5f, -63.21f, 17.93f);
            Vec3f scale(1.f, 2.f, 3.f);
            Mat4 transform = Mat4::CreateTransformMatrix(translation, euler, scale);
            REQUIRE(hasher(Mat4::Identity()) != hasher(transform));
            REQUIRE(hasher(Mat4(1.f)) != hasher(Mat4(2.f)));

            // Usable as unordered_map key
            std::unordered_map<Mat4, std::string> map;
            Mat4 translation4 = Mat4::CreateTranslationMatrix(translation);
            Mat4 rotation4 = Mat4::CreateRotationMatrix(euler);
            map[Mat4::Identity()] = "identity";
            map[translation4] = "translation";
            map[rotation4] = "rotation";
            map[transform] = "transform";
            REQUIRE(map[Mat4::Identity()] == std::string("identity"));
            REQUIRE(map[translation4] == std::string("translation"));
            REQUIRE(map[rotation4] == std::string("rotation"));
            REQUIRE(map[transform] == std::string("transform"));
            COMPARE((int)map.size(), 4);
        }

        TEST(Hash_Unordered_Set)
        {
            // Verify all types work with unordered_set
            std::unordered_set<Vec2f> vec2Set = {Vec2f(1, 2), Vec2f(3, 4), Vec2f(1, 2)};
            std::unordered_set<Vec3f> vec3Set = {Vec3f(1, 2, 3), Vec3f(4, 5, 6), Vec3f(1, 2, 3)};
            std::unordered_set<Vec4f> vec4Set = {Vec4f(1, 2, 3, 4), Vec4f(5, 6, 7, 8), Vec4f(1, 2, 3, 4)};
            std::unordered_set<Quat> quatSet = {Quat::Identity(), Quat(1, 0, 0, 0), Quat::Identity()};
            std::unordered_set<Mat3> mat3Set = {Mat3::Identity(), Mat3(2.f), Mat3::Identity()};
            std::unordered_set<Mat4> mat4Set = {Mat4::Identity(), Mat4(2.f), Mat4::Identity()};

            // Duplicates must be collapsed
            COMPARE((int)vec2Set.size(), 2);
            COMPARE((int)vec3Set.size(), 2);
            COMPARE((int)vec4Set.size(), 2);
            COMPARE((int)quatSet.size(), 2);
            COMPARE((int)mat3Set.size(), 2);
            COMPARE((int)mat4Set.size(), 2);
        }
    }
#pragma endregion
}
